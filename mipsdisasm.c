...
static void link_with_lui(disasm_state *state, int block_id, int offset, unsigned int reg, unsigned int mem_imm, unsigned int vaddr, char *prefix)
{
            ...
            if (reg == rd) {
               ...
               if (insn[offset].id != MIPS_INS_ORI) {
                  ...
                  if (label < 0) {
					 ...
					 if(addr >= vaddr)
					 {
						sprintf(label_name, "%s_D_%08X", prefix, addr);
...
static void disassemble_block(unsigned char *data, unsigned int length, unsigned int vaddr, disasm_state *state, int block_id, char *prefix)
{
   asm_block *block = &state->blocks[block_id];
         ...
         if (insn[i].is_jump) {
            ...
            if (insn[i].id == MIPS_INS_JAL || insn[i].id == MIPS_INS_BAL || insn[i].id == MIPS_INS_J) {
               ...
               if (labels_find(&state->globals, jal_target) < 0) {
                  char label_name[32];
				  if(jal_target >= vaddr && prefix)
				  {
					sprintf(label_name, "%s_func_%08X", prefix, jal_target);
               ...
               for (int o = 0; o < insn[i].op_count; o++) {
                  if (insn[i].operands[o].type == MIPS_OP_IMM) {
                     ...
                     if (label < 0) {
						if(branch_target >= vaddr && prefix)
						{
							switch (state->syntax) {
							   case ASM_GAS:    sprintf(label_name, ".%s_L%08X", prefix, branch_target); break;
							   case ASM_ARMIPS: sprintf(label_name, "@%s_L%08X", prefix, branch_target); break;
							}
						}
						else
						{
         ...
         if (state->merge_pseudo) {
            switch (insn[i].id) {
               ...
               case MIPS_INS_SWC3:
               {
                  ...
                  link_with_lui(state, block_id, i, mem_rs, mem_imm, vaddr, prefix);
                  break;
               }
               case MIPS_INS_ADDIU:
               case MIPS_INS_ORI:
               {
                  ...
                  if (rs == MIPS_REG_ZERO) { // becomes LI
                     ...
                     link_with_lui(state, block_id, i, rs, (unsigned int)imm, vaddr, prefix);
                  }
                  break;
               }
            }
...
void mipsdisasm_pass1(unsigned char *data, unsigned int offset, unsigned int length, unsigned int vaddr, disasm_state *state, char *prefix)
{
   ...
   disassemble_block(&data[offset], length, vaddr, state, state->block_count, prefix);
...
static void print_usage(void)
{
   ERROR("Usage: mipsdisasm [-o OUTPUT] [-p] [-s ASSEMBLER] [-v] ROM [RANGES]\n"
		 ...
		 " -i PREFIX    internal symbol prefix.\n"
         "\n"
         "Arguments:\n"
...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < args.range_count; i++) {
      ...
      (void)mipsdisasm_pass1(data, r->start, r->length, r->vaddr, state, args.prefix);
   }
